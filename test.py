...
def validate_instruments(args, model, criterion, test_data):
    dataloader = torch.utils.data.DataLoader(test_data,
                                             batch_size=args.batch_size,
                                             shuffle=False,
                                             num_workers=args.num_workers)
    ...
    model.eval()
    total_loss = 0.
    test_loss_dict = {inst: 0.0 for inst in args.instruments}
    with tqdm(total=len(test_data) // args.batch_size) as pbar, torch.no_grad():
        for example_num, (x, targets) in enumerate(dataloader):
            if args.cuda:
                x = x.cuda()
            ...
            outputs, avg_loss = model_utils.compute_loss(model, x, targets, criterion)
            avg_loss_dict = model_utils.compute_avg_loss_dict(model, outputs, targets, criterion)
            # avg_loss_dict = model_utils.compute_loss(model, x, targets, criterion)

            for inst in args.instruments:
                test_loss_dict[inst] += (1. / float(example_num + 1)) * (avg_loss_dict[inst] - test_loss_dict[inst])
